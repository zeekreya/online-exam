...
const StudentResult = () => {
  ...
  const getUserResult = async () => {
    try {
      let url = `http://localhost:3000/result/exam-result?userId=${JSON.parse(localStorage.getItem("userData"))?._id}`;
      if (filterExamId) {
        url = `http://localhost:3000/result/exam-result?userId=${JSON.parse(localStorage.getItem("userData"))?._id}&examId=${filterExamId}`;
      }
